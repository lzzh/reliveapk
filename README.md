...
| **文字自动排版** | 复用服务端渲染好的文字条（AI 文案 + 日期 + 地点），无需 App 内造字体 |
...
| 概念 | 是什么 | 是否全彩高清 |
|---|---|---|
| **原图模式（本 App 默认）** | 直接取服务器 `photos/{id}/image` 的**原始照片** | ✅ 是，全彩高清，跟你在相册里看到的一样 |
| **相框规格** | Relive 服务端按设备渲染的 **480×800 墨水屏位图**，仅给墨水屏相框用 | ❌ 是降色（4 色 / 6 色）墨屏稿 |

**关键**：4 色 / 6 色（`gdem075f52_480x800_4color`、`spectra6_480x800`）是服务端给**墨水屏相框**的渲染档位，由你在 Relive 后台给设备选的 `RenderProfile` 决定。

本 App **走原图模式，完全不依赖它们** → 永远全彩高清。

> 你如果曾在 App 里看到「6 色 / 不高清」，那说明 App **回退到了相框位图**（原图取图失败）。
> 本版修复后只要原图能取到，就是高清全彩。
...
| 取原图 | `GET /api/v1/photos/{photo_id}/image` | 原始照片（实测可达 3840×5760） |
